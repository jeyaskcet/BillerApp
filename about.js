...
billStatusElement.value = paymentStatus;
    ...
    const billId = 'BillHistory_' + `${customerName}-${billNumber}`;
...
function navigateTobillHistoryPage() {
  showLoading();
  setTimeout(function() {
    window.location.href = "services.html"; // Replace with the actual logout page URL
  }, 2000); // 3000 milliseconds = 3 seconds
}
...
function suggestItem_old() {
  ...
  if (userInput !== "") { // Check if userInput is not empty
    ...
    function suggestItem() {
        ...
        if (userInput !== "") {
            // Retrieve the savedUserItems data from localStorage
            const savedUserItems = JSON.parse(localStorage.getItem("userItems")) || [];

// Fetch predefinedItems from local storage
const predefinedItems = JSON.parse(localStorage.getItem("predefinedItems")) || [];

// Combine predefinedItems and savedUserItems
        const allItems = [...predefinedItems, ...savedUserItems];
